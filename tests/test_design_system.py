...
def test_compact_coachbook_dashboard_assets(tmp_path: Path) -> None:
    ...
    assert ".coach-os__grid" in coach_css.text
    assert ".strategy-panel--evidence" in coach_css.text
    ...
    assert "[data-insight-tab]" in coach_js.text
    assert "[data-insight-panel]" in coach_js.text
    assert 'addEventListener("keydown"' in coach_js.text
    assert 'event.key === "ArrowRight"' in coach_js.text
    assert tokens.status_code == 200
