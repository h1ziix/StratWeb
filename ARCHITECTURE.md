...
## Stage 9.6.5 architectural decision — plain language is a projection

The coach view never rewrites persisted Tactical insights. It maps an exact frequency to one
deterministic presentation band: every eligible case, almost always, often, sometimes, rarely or
not seen. Reliability wording is derived only from `match_count` and the persisted
`small_sample_warning`; it is not generated by an LLM and does not claim statistical support that
the source model lacks.

The unfiltered screen deliberately selects the highest-sample representative of each insight
family and limits the headline area to three families. An explicit family filter returns the full
persisted set, so curation is reversible and never deletes evidence. Raw rotation keys and heatmap
coordinates no longer serve as prose; precise spatial evidence remains reachable from the detail
page.

Numerator, denominator, exact frequency, match/evidence counts, limitations, ticks, UUIDs,
