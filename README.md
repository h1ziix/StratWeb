...
Обновление 0.29.0 — Coachbook Dashboard: Стратбук перестроен в компактную рабочую
панель по утверждённому визуальному макету. Выводы, атака, защита, рекомендации,
мини-карта, локальный AI-пересказ и доказательства теперь доступны на одном экране
без обязательного пошагового режима. Бэкенд и расчёты не изменялись. Подробности:
[STAGE_9_15_1.md](STAGE_9_15_1.md).
...
за портом), DuckDB persistence (миграции 001–034), `Gameplay Analytics Engine V1`
...
Для быстрой подготовки доступна одностраничная шпаргалка на выбранную карту: короткие
ожидания за T/CT, риски, готовые ответы, копирование в командный чат и печать без
технических идентификаторов ([MATCH_CHEAT_SHEET.md](MATCH_CHEAT_SHEET.md)).
...
Head-to-Head сопоставляет привычки соперника с доказанными привычками нашей команды
на той же карте и за противоположные стороны ([HEAD_TO_HEAD.md](HEAD_TO_HEAD.md)).
Профили соперников разделены на командные и личные: личный стратбук закрепляет один
Steam ID, скрывает командные выводы, показывает подтверждённые ранние позиции игрока и
экспортируется в JSON, печать и PDF ([STAGE_9_12.md](STAGE_9_12.md)).
Командный тактический обзор и шпаргалка показывают доказательную CT-расстановку:
опорников A/B, игрока мида/снайпера и ротатора с долей подтверждённых раундов
([STAGE_9_13.md](STAGE_9_13.md)).
Перед открытием простого стратбука пользователь выбирает карту по карточке с официальным
локальным CS2 overview; личные и командные выводы после этого не смешиваются между картами
([STAGE_9_13_1.md](STAGE_9_13_1.md)).
...
Если тактический обзор готов, а план ещё не собран, нажмите **«Подготовить план на матч»**.
Приложение последовательно рассчитывает повторения, наблюдения и ответы по уже обработанным
матчам. Затем открывает сохранённый план. Повторное нажатие не дублирует одинаковые расчёты.
Если не готов ни один матч, страница объяснит, что нужно завершить обработку, и даст ссылки
на выбранные матчи. Само открытие страницы ничего не пересчитывает.

Подготовка: localhost-only `POST /ui/opponents/{profile_id}/report/prepare`.
Этот шаг не парсит демки заново и не снижает требования к доказательствам или надёжности.
Неготовые матчи исключаются явно; отсутствие подтверждённых советов не заменяется догадками.
